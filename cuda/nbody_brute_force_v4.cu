...
__global__ void compute_forces(float *m, float*fx, float*fy, float *px, float *py,int nparticles){
  ...
  sm[threadIdx.y][threadIdx.x] = 0;
  sm[32+threadIdx.y][32+threadIdx.x] = 0;
  ...
  //perform reduction
  sm[threadIdx.y][threadIdx.x] = grav_base*x_sep;
  sm[32+threadIdx.y][32+threadIdx.x] = grav_base*y_sep;
  __syncthreads();

  for (unsigned int s=16; s>0; s>>=1){
    if(threadIdx.x < s){
      sm[threadIdx.y][threadIdx.x] += sm[threadIdx.y][threadIdx.x+s];
      sm[32+threadIdx.y][32+threadIdx.x] += sm[32+threadIdx.y][32+threadIdx.x+s];
    }
    __syncthreads();
  }

  if(threadIdx.y == 0){
    atomicAdd(&fx[x], sm[threadIdx.y][0]);
    atomicAdd(&fy[x], sm[32+threadIdx.y][32]);
  }
  __syncthreads();
