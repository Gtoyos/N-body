...
__global__ void compute_forces(particle_t* gprt, int nparticles){
  ...
  if(x<y) return; //only compute half of the matrix (symmetric)

  particle_t *ip = &gprt[x];
  particle_t *jp = &gprt[y];

  float x_sep = jp->x_pos - ip->x_pos;
  float y_sep = jp->y_pos - ip->y_pos;
  ...
  atomicAdd(&(jp->x_force), -grav_base*x_sep);
  atomicAdd(&(jp->y_force), -grav_base*y_sep);
}
...
void all_move_particles(float step){
  ...
  //compute forces.
  compute_forces<<<grid2, block2>>>(g_particles, nparticles);
  gpuErrchk( cudaPeekAtLastError() );
  cudaDeviceSynchronize();
  //move each particle.
