...
__global__ void compute_forces(particle_t* gprt,float* g_x_pos,float *g_y_pos,int local_np,int delta0,int nparticles){
  int x = blockIdx.x * blockDim.x + threadIdx.x;
  int y = blockIdx.y * blockDim.y + threadIdx.y;
  if(x >= nparticles) return;
  if(y >= delta0+local_np) return;
  particle_t *ip = &gprt[delta0+y]; //forces i will save
  particle_t *jp = &gprt[x];

  float x_sep = g_x_pos[x] - g_x_pos[delta0+y];
  float y_sep = g_y_pos[x] - g_y_pos[delta0+y];
...
__global__ void move_particles(particle_t* gprt,float *g_local_x_pos, float *g_local_y_pos,
    int local_np, int delta0, float step,float *gmacc, float *gms, float *gssq){
  int x = blockIdx.x * blockDim.x + threadIdx.x;
  if(x >= local_np) return;
...
void all_move_particles(float step){
  ...
  dim3 block(1024);
  dim3 grid2(nparticles/32+1, local_np/32+1);
  dim3 block2(32,32);
  ...
  //compute forces.
  compute_forces<<<grid2, block2>>>(g_particles,g_x_pos,g_y_pos,local_np,delta0,nparticles);
  gpuErrchk( cudaPeekAtLastError() );
  cudaDeviceSynchronize();
  //move each particle.
